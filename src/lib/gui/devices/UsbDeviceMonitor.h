...
public:
   ...
   */
  QString vendorIdFilter() const { return m_vendorIdFilter; }

  /**
   * @brief Get current product ID filter
   */
  QString productIdFilter() const { return m_productIdFilter; }
...
protected:
  /**
   * @brief Check if a device matches the configured filters
   * @param device Device to check
   * @return true if device matches filters (or no filters set)
   */
